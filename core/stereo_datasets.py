...
class SceneFlowDatasets(StereoDataset):
    ...
    def _add_things(self, split='TRAIN'):
        ...
        logging.info(
            f"Added {len(self.disparity_list) - original_length} from FlyingThings {self.dstype}"
        )
    ...
    def _add_monkaa(self):
        ...
        for img1, img2, disp in zip(left_images, right_images,
            ...
            self.disparity_list += [disp]
        logging.info(
            f"Added {len(self.disparity_list) - original_length} from Monkaa {self.dstype}"
        )
    ...
    def _add_driving(self):
        ...
        for img1, img2, disp in zip(left_images, right_images,
            ...
            self.disparity_list += [disp]
        logging.info(
            f"Added {len(self.disparity_list) - original_length} from Driving {self.dstype}"
        )
...
class Middlebury(StereoDataset):
    ...
    def __init__(self, aug_params=None, root='datasets/Middlebury', split='F'):
        ...
        if split == "2014":  # datasets/Middlebury/2014/Pipes-perfect/im0.png
            ...
            assert len(image1_list) == len(image2_list) == len(
                disp_list) > 0, [image1_list, split]
            for img1, img2, disp in zip(image1_list, image2_list, disp_list):
                self.image_list += [[img1, img2]]
                self.disparity_list += [disp]
